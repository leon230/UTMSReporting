...
/* 		(
	...
	/* 	TO_CHAR(UTC.GET_LOCAL_DATE(ORLS.LATE_PICKUP_DATE,ORLS.SOURCE_LOCATION_GID),'YYYY-MM-DD HH24:MI')                                                LATE_PICKUP_DATE,
		...
		(SELECT DISTINCT
		(CASE WHEN SS.STOP_TYPE ='D' THEN (UTC.GET_LOCAL_DATE(IESH_EVENT_DELIVER.EVENTDATE,ORLS.DEST_LOCATION_GID))
		WHEN SS.STOP_TYPE ='P'THEN (UTC.GET_LOCAL_DATE(IESH_EVENT_DELIVER.EVENTDATE,ORLS.SOURCE_LOCATION_GID)) END)
...
/* 		(SELECT DISTINCT MIN(IESH_EVENT_DELIVER.INSERT_DATE)
		...
		) AS																																LATEST_P_D_VS_STATUS_INSERT,	
		----------------------
		CASE WHEN (
		...
		) >0 THEN 'Y' ELSE 'N' END AS																										BOOKING_VS_LATES_PICK,
        ...
        WHERE  IESH_EVENT_DELIVER_TEMP.STATUS_CODE_GID ='ULE/PR.0008'
               AND SSH_TEMP.SHIPMENT_GID = SH.SHIPMENT_GID 
			   AND SSH_TEMP.SHIPMENT_STOP_NUM = SS.STOP_NUM))-
		(SELECT IESH_EVENT_DELIVER.EVENTDATE
        FROM   SS_STATUS_HISTORY SSH 
               LEFT OUTER JOIN IE_SHIPMENTSTATUS IESH_EVENT_DELIVER 
                            ON ( SSH.I_TRANSACTION_NO = 
                                 IESH_EVENT_DELIVER.I_TRANSACTION_NO ) 
		...
		) >0 THEN 'Y' ELSE 'N' END AS																											REG_OUT_VS_REG_IN,
...
WHERE 1=1
	...
	AND	SH.SHIPMENT_TYPE_GID IN ('TRANSPORT','APPOINTMENT')
