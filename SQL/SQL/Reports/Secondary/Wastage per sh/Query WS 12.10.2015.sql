...
trim(to_char(sum(Number_of_Shipments),'999999999D99','NLS_NUMERIC_CHARACTERS = '', ''')) as Number_of_Shipments,
trim(to_char(sum(Total_Number_of_PFS),'999999999D99','NLS_NUMERIC_CHARACTERS = '', ''')) as Total_Number_of_PFS,
trim(to_char(sum(Total_Weight_Normalized),'999999999D99','NLS_NUMERIC_CHARACTERS = '', ''')) as Total_Weight_Normalized,
trim(to_char(sum(Total_Ord_Cost_B_EUR),'999999999D99','NLS_NUMERIC_CHARACTERS = '', ''')) as Total_Ord_Cost_B_EUR,
trim(to_char(sum(Total_Ord_Cost_Base_EUR),'999999999D99','NLS_NUMERIC_CHARACTERS = '', ''')) as Total_Ord_Cost_Base_EUR,
WEIGHT_LANE
...
when sh.SERVPROV_GID = 'ULE.T80278' and ord_rel.SOURCE_LOCATION_GID = 'ULE.V214422' then 'Y'
  when sh.SERVPROV_GID = 'ULE.T1019034' and ord_rel.SOURCE_LOCATION_GID = 'ULE.V214466' then 'Y'
  when sh.SERVPROV_GID = 'ULE.T4910526' and ord_rel.SOURCE_LOCATION_GID = 'ULE.V214481' then 'Y'
  when sh.SERVPROV_GID = 'ULE.T4100237' and ord_rel.SOURCE_LOCATION_GID = 'ULE.V214422' then 'Y'
  when sh.SERVPROV_GID = 'ULE.T1030444' and ord_rel.SOURCE_LOCATION_GID = 'ULE.V214466' then 'Y'
...
trim(sum(nvl(round((select sum(CASE
...
-- and 
-- (al_ord_rel_d1.accessorial_code_gid is null or rpt_general.f_remove_domain(al_ord_rel_d1.accessorial_code_gid) IN ('ULE_MYTO','ULE_TOLL','ULE_FTL FREIGHT COST CORRECTION','ULE_BONUS_MALUS','ULE_LTL FREIGHT COSTS CORRECTION','HAZARDOUS','ULE_FREIGHT COST BASIC LTL PER PALLET','ULE_FREIGHT COST BASIC FTL','ULE_TANK_CHASSIS HIRE','ULE_FTL FREIGHT COST CORRECTION','ULE_LTL FREIGHT COSTS CORRECTION','ULE_FREIGHT COST BASIC FTL','ULE_FREIGHT COST BASIC LTL PER PALLET','ULE_BONUS_MALUS','ULE_TOLL'))
