...
,temp.source_city																								SOURCE_CITY
,temp.source_location_gid																						SOURCE_LOCATION_GID
...
,SUM(temp.pfs)/COUNT(temp.shipment_gid)																																		PALLETS_PER_TRUCK
,SUM(temp.PALLET_GROSS_WEIGHT_KG)/COUNT(temp.shipment_gid)																													WEIGHT_PER_TRUCK
,ROUND((SUM(TEMP.PFS)/SUM(TEMP.TRUCK_CAPACITY_PFS)),2)																														PFS_PERCENTAGE
,ROUND((SUM(TEMP.PALLET_GROSS_WEIGHT_KG)/SUM(TEMP.TRUCK_CAPACITY_WEIGHT)),2)																								WEIGHT_PERCENTAGE

,case when ROUND((SUM(TEMP.PFS)/SUM(TEMP.TRUCK_CAPACITY_PFS)),2) > ROUND((SUM(TEMP.PALLET_GROSS_WEIGHT_KG)/SUM(TEMP.TRUCK_CAPACITY_WEIGHT)),2)
THEN TRIM(TO_CHAR(((1-ROUND((SUM(TEMP.PFS)/SUM(TEMP.TRUCK_CAPACITY_PFS)),2)) * SUM(temp.total_cost_eur)),'9999999999990.0'))
else TRIM(TO_CHAR(((1-ROUND((SUM(TEMP.PALLET_GROSS_WEIGHT_KG)/SUM(TEMP.TRUCK_CAPACITY_WEIGHT)),2)) * SUM(temp.total_cost_eur)),'9999999999990.0'))	 END  				PFS_WASTAGE
-------------------YTD------------------------------------------
...
,ROAD_DATA AS (
SELECT /*+ FULL(sh) */sh.shipment_gid
...
SELECT /*+ FULL(sh) LEADING(sh rd) */ sh.shipment_gid																							SHIPMENT_GID
...
,MAX(PFS_WASTAGE) AS PFS_WASTAGE
