SELECT DISTINCT sh.shipment_gid,
	   om.order_release_gid,
		...
		-- ((TRIM(TO_CHAR(replace(orl_ref_pal.ORDER_RELEASE_REFNUM_VALUE,',','.'),'999999999D99','NLS_NUMERIC_CHARACTERS = '', '''))))  end),
		-- '999999999D99','NLS_NUMERIC_CHARACTERS = '', ''')))	> 20 
		-- THEN 
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
coalesce(trim(to_char((select min(temp2.price)
		...
		(SELECT LOC.CITY
		FROM LOCATION LOC
		WHERE LOC.LOCATION_GID = temp2.source_location_gid
		...
		(SELECT LOC.CITY
		FROM LOCATION LOC
		WHERE LOC.LOCATION_GID = orls_pal.source_location_gid
		)
		AND 
		(SELECT LOC.CITY
		FROM LOCATION LOC
		WHERE LOC.LOCATION_GID = temp2.dest_LOCATION_GID
		...
		(SELECT LOC.CITY
		FROM LOCATION LOC
		WHERE LOC.LOCATION_GID = orls_pal.dest_location_gid
		)
		...
		(select distinct temp.SOURCE_LOCATION_GID,
							...
							(SELECT  
									...
									AND XL.X_LANE_GID = RG.X_LANE_GID
									and xl.domain_name = 'ULE/PR'
									and trunc(sysdate) between rg.EFFECTIVE_DATE and rg.EXPIRATION_DATE
											) temp
							group by 
							temp.SOURCE_LOCATION_GID,
							temp.DEST_LOCATION_GID,
...
--		and trunc(sysdate) between temp2.EFFECTIVE_DATE and temp2.EXPIRATION_DATE

		and 
		(CEIL(TO_NUMBER((case when REGEXP_LIKE(temp2.PALLET_AMOUNT, '[^0-9 \.,]') then '0'
		else 
		
		((TRIM(TO_CHAR(replace(temp2.PALLET_AMOUNT,',','.'),'999999999D99','NLS_NUMERIC_CHARACTERS = '', '''))))  end),
		'999999999D99','NLS_NUMERIC_CHARACTERS = '', ''')))=
		(CEIL(TO_NUMBER((case when REGEXP_LIKE(orl_ref_pal.ORDER_RELEASE_REFNUM_VALUE, '[^0-9 \.,]') then '0'
		else 
		...
		FROM LOCATION LOC
		WHERE LOC.LOCATION_GID = sh.SOURCE_LOCATION_GID)		SOURCE_LOC_COUNTRY,
		(SELECT LOC.LOCATION_NAME
		FROM LOCATION LOC
		...
		(SELECT LOC.COUNTRY_CODE3_GID
		FROM LOCATION LOC
		WHERE LOC.LOCATION_GID = sh.DEST_LOCATION_GID)		DEST_LOC_COUNTRY,
