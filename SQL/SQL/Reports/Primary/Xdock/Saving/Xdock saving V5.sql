SELECT DISTINCT sh.shipment_gid,
	   om.order_release_gid,
		...
		),'999999999D99','NLS_NUMERIC_CHARACTERS = ''. ''')||'_LTL') 													direct_price_ltl_max,
	...
	trim(to_char((CASE WHEN 
		...
		END),'999999999D99','NLS_NUMERIC_CHARACTERS = ''. ''')||'_FTL')																					PRICE_FTL_min,
		trim(to_char((CASE WHEN 
		...
		END),'999999999D99','NLS_NUMERIC_CHARACTERS = ''. ''')||'_FTL')																					PRICE_FTL_max,
		...
		(SELECT LOC.COUNTRY_CODE3_GID
		FROM LOCATION LOC
		WHERE LOC.LOCATION_GID = sh.DEST_LOCATION_GID)then 'National'
		else 'International'
