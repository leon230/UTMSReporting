...
(SELECT DISTINCT ORLS.SOURCE_LOCATION_GID AS																											SOURCE_LOC_ID,
	...
	NVL(rpt_general.f_remove_domain(SH.RATE_GEO_GID),'-')		 																											LANE_ID,
	...
	NVL((SELECT R_GEO_REM.REMARK_TEXT
	 ...
	 WHERE
		...
		AND R_GEO.RATE_GEO_GID = R_GEO_REM.RATE_GEO_GID),'-')																												SHIPMENTS_PER_YEAR,
	...
	FROM
		RATE_GEO_COST RGC
	WHERE
		RGC.RATE_GEO_COST_GROUP_GID = SH.PLANNED_RATE_GEO_GID),																					
	(SELECT
		CASE
	...
	WHERE
		...
		AND RUB.RATE_UNIT_BREAK_MAX = 
			...
			WHERE SH_REF.SHIPMENT_REFNUM_QUAL_GID = 'ULE.ULE_ORIGINAL_PFS' AND SH_REF.SHIPMENT_GID = SH.SHIPMENT_GID)),'-')													LANE_RATE_EUR,			
	...
	WHERE L_ATTR.LANE_ATTRIBUTE_DEF_GID = 'ULE.ULE_NOTIFICATION_TIME_H'
		...
		AND R_GEO.RATE_GEO_GID = SH.RATE_GEO_GID),'-')																														NOTIFICATION_TIME_H,

	NVL((SELECT TO_CHAR(SERVICE_TIME_VALUE/3600)
	FROM SERVICE_TIME SERV_TIME,
		RATE_GEO R_GEO
	WHERE SERV_TIME.X_LANE_GID = R_GEO.X_LANE_GID
		AND R_GEO.RATE_GEO_GID = SH.RATE_GEO_GID),'-')																														TRANSIT_TIME_HH,
...
FROM
	...
	WHERE
		...
		and SH_T_COLL_BOV.SHIPMENT_GID='ULE/PR.100842870') T_DECLINE,
...
WHERE
	...
	and sh.shipment_gid = 'ULE/PR.100842870'
