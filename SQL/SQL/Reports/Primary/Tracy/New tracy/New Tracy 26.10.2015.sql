...
WHERE 1=1
	...
	-- (select NVL(SR_REGION.SHIPMENT_REFNUM_VALUE,'N/A') AS REGION
		...
		-- where SR_REGION.SHIPMENT_GID = sh.SHIPMENT_GID AND SR_REGION.SHIPMENT_REFNUM_QUAL_GID = 'ULE.ULE_FUNCTIONAL_REGION' and rownum = 1
		-- ),:P_REGION)
		AND NVL((SELECT (LISTAGG(SH_REG.SHIPMENT_REFNUM_VALUE,'/ ') WITHIN GROUP (ORDER BY SH_REG.SHIPMENT_REFNUM_VALUE))
				FROM SHIPMENT_REFNUM SH_REG
				WHERE SH.SHIPMENT_GID = SH_REG.SHIPMENT_GID
					  AND SH_REG.SHIPMENT_REFNUM_QUAL_GID = 'ULE.ULE_FUNCTIONAL_REGION'),'-') =
					  
				
				
				(CASE WHEN :P_REGION = 'ALL' THEN 
					NVL((SELECT (LISTAGG(SH_REG.SHIPMENT_REFNUM_VALUE,'/ ') WITHIN GROUP (ORDER BY SH_REG.SHIPMENT_REFNUM_VALUE))
				FROM SHIPMENT_REFNUM SH_REG
				WHERE SH.SHIPMENT_GID = SH_REG.SHIPMENT_GID
					  AND SH_REG.SHIPMENT_REFNUM_QUAL_GID = 'ULE.ULE_FUNCTIONAL_REGION'),'-')
				ELSE 
