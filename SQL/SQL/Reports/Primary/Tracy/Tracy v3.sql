...
(SELECT DISTINCT ORLS.SOURCE_LOCATION_GID AS																														SOURCE_LOC_ID,
		...
		trunc(orls.early_pickup_date),
		to_char(nvl(:START_TIME,orls.early_pickup_date),:P_DATE_TIME_FORMAT)
...
/* 		and	orls.early_pickup_date between to_date(nvl(:START_TIME, orls.early_pickup_date),:P_DATE_TIME_FORMAT) AND 
	to_date(NVL(:END_TIME,orls.early_pickup_date),:P_DATE_TIME_FORMAT)  */
		
		/* and trunc(orls.early_pickup_date) = trunc(to_date(nvl(:START_TIME, trunc(orls.early_pickup_date)),:P_DATE_TIME_FORMAT)) */
UNION ALL
(
SELECT SH.SOURCE_LOCATION_GID AS																														SOURCE_LOC_ID,
		...
		trunc(sh.start_time),
		to_char(nvl(:START_TIME,sh.start_time),:P_DATE_TIME_FORMAT)
...
WHERE 1=1	
	...
	AND SH.SOURCE_LOCATION_GID = nvl(:SOURCE_LOC,SH.SOURCE_LOCATION_GID)
		...
		/* and trunc(sh.start_time) = to_date(nvl(:START_TIME, trunc(sh.start_time)),:P_DATE_TIME_FORMAT) */
