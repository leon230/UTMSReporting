...
AND sr_region.SHIPMENT_REFNUM_QUAL_GID = 'ULE.ULE_FUNCTIONAL_REGION') ) as 																						SH_REGION,
	...
	nvl((SELECT 
		LISTAGG(or_ref_1.ORDER_RELEASE_REFNUM_VALUE,'/') WITHIN GROUP (ORDER BY or_ref_1.ORDER_RELEASE_GID)
	FROM 
	...
	nvl((SELECT 
		LISTAGG(or_ref_2.ORDER_RELEASE_REFNUM_VALUE,'/') WITHIN GROUP (ORDER BY or_ref_2.ORDER_RELEASE_GID)
	FROM 
...
WHERE
	...
	AND trunc(SH.END_TIME)  <= 
	case when :P_END_DATE is not null
	then to_date(:P_END_DATE,:P_DATE_TIME_FORMAT)
...
and 
	-- (select NVL(LOC_REF.LOCATION_REFNUM_VALUE,'N/A') AS REGION
		-- from LOCATION_REFNUM LOC_REF 
		-- where LOC_REF.location_gid = sh.servprov_gid AND LOC_REF.LOCATION_REFNUM_QUAL_GID = 'ULE.ULE_RESPONSIBLE_REGION'
		-- ) =

		(select NVL(LOC_REF.LOCATION_REFNUM_VALUE,'N/A') AS REGION
		from 
		LOCATION LOC LEFT OUTER JOIN 
		LOCATION_REFNUM LOC_REF ON (LOC.LOCATION_GID = LOC_REF.LOCATION_GID AND LOC_REF.LOCATION_REFNUM_QUAL_GID = 'ULE.ULE_RESPONSIBLE_REGION')
		where LOC.location_gid = sh.servprov_gid 
		) =

		decode(:P_RESP_REGION,'ALL',(select NVL(LOC_REF.LOCATION_REFNUM_VALUE,'N/A') AS REGION
		from 
		LOCATION LOC LEFT OUTER JOIN 
		LOCATION_REFNUM LOC_REF ON (LOC.LOCATION_GID = LOC_REF.LOCATION_GID AND LOC_REF.LOCATION_REFNUM_QUAL_GID = 'ULE.ULE_RESPONSIBLE_REGION')
		where LOC.location_gid = sh.servprov_gid),:P_RESP_REGION)
