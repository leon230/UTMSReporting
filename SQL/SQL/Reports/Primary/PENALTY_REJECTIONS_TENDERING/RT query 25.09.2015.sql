SELECT DISTINCT
	...
	WHERE
		...
		AND RUB.RATE_UNIT_BREAK_MAX = 
			(SELECT FLOOR((TO_NUMBER((case when REGEXP_LIKE(SH_REF.shipment_REFNUM_VALUE, '[^0-9 \.]') then '0'
		else 
		
		((TRIM(TO_CHAR(replace(SH_REF.shipment_REFNUM_VALUE,',','.'),'999999999D99','NLS_NUMERIC_CHARACTERS = '', '''))))  end),
...
(SELECT count(SH_T_COLL_BOV2.SHIPMENT_GID)											SH_ID
	...
	((SELECT MIN(ORLS.early_pickup_date)
	FROM ORDER_RELEASE ORLS,
		SHIPMENT_STOP SS,
		SHIPMENT_STOP_D SSD,
		S_SHIP_UNIT SSU,
	...
	WHERE SS.SHIPMENT_GID = SH.SHIPMENT_GID
		...
		AND SSD.S_SHIP_UNIT_GID = SSU.S_SHIP_UNIT_GID
		AND SSU.S_SHIP_UNIT_GID = SSUL.S_SHIP_UNIT_GID
		AND SSUL.ORDER_RELEASE_GID = ORLS.ORDER_RELEASE_GID)-T_DECLINE.TENDER_DATE)*24,'999999999999D99','NLS_NUMERIC_CHARACTERS = '', ''') )											early_pick_vs_Received,
		
		
		trim(to_char(((SELECT MIN(ORLS.early_DELIVERY_DATE)
	FROM ORDER_RELEASE ORLS,
		SHIPMENT_STOP SS,
		SHIPMENT_STOP_D SSD,
		S_SHIP_UNIT SSU,
	...
	WHERE SS.SHIPMENT_GID = SH.SHIPMENT_GID
		...
		AND SSD.S_SHIP_UNIT_GID = SSU.S_SHIP_UNIT_GID
		AND SSU.S_SHIP_UNIT_GID = SSUL.S_SHIP_UNIT_GID
		AND SSUL.ORDER_RELEASE_GID = ORLS.ORDER_RELEASE_GID) - (SELECT MAX(ORLS.early_pickup_date)
	FROM ORDER_RELEASE ORLS,
		SHIPMENT_STOP SS,
		SHIPMENT_STOP_D SSD,
		S_SHIP_UNIT SSU,
	...
	WHERE SS.SHIPMENT_GID = SH.SHIPMENT_GID
		...
		AND SSD.S_SHIP_UNIT_GID = SSU.S_SHIP_UNIT_GID
		AND SSU.S_SHIP_UNIT_GID = SSUL.S_SHIP_UNIT_GID
		AND SSUL.ORDER_RELEASE_GID = ORLS.ORDER_RELEASE_GID))*24
		
		 ,'999999999999D99','NLS_NUMERIC_CHARACTERS = '', '''))																										early_del_vs_early_pick,
		...
		and trunc(sh_temp.start_time) = trunc(
		From_tz(cast((SELECT MIN(ORLS.early_pickup_date)
	FROM ORDER_RELEASE ORLS,
		SHIPMENT_STOP SS,
		SHIPMENT_STOP_D SSD,
		S_SHIP_UNIT SSU,
	...
	WHERE SS.SHIPMENT_GID = SH.SHIPMENT_GID
		...
		AND SSD.S_SHIP_UNIT_GID = SSU.S_SHIP_UNIT_GID
		AND SSU.S_SHIP_UNIT_GID = SSUL.S_SHIP_UNIT_GID
		AND SSUL.ORDER_RELEASE_GID = ORLS.ORDER_RELEASE_GID) AS TIMESTAMP), 'GMT') AT TIME ZONE 'CET')  ------
		...
		where
		sh_temp.servprov_gid = t_decline.TSP_ID
		and to_char(trunc(sh_temp.start_time),'WW') = to_char(trunc(
		From_tz(cast((SELECT MIN(ORLS.early_pickup_date)
	FROM ORDER_RELEASE ORLS,
		SHIPMENT_STOP SS,
		SHIPMENT_STOP_D SSD,
	...
	WHERE SS.SHIPMENT_GID = SH.SHIPMENT_GID
		...
		AND SSD.S_SHIP_UNIT_GID = SSU.S_SHIP_UNIT_GID
		AND SSU.S_SHIP_UNIT_GID = SSUL.S_SHIP_UNIT_GID
		AND SSUL.ORDER_RELEASE_GID = ORLS.ORDER_RELEASE_GID) AS TIMESTAMP), 'GMT') AT TIME ZONE 'CET'),'WW') 
		and sh_temp.rate_geo_gid = T_DECLINE.RR_ID
		...
		where
		sh_temp.servprov_gid = t_decline.TSP_ID
		and to_char(trunc(sh_temp.start_time),'MM') = to_char(trunc(
		From_tz(cast((SELECT MIN(ORLS.early_pickup_date)
	FROM ORDER_RELEASE ORLS,
		SHIPMENT_STOP SS,
		SHIPMENT_STOP_D SSD,
	...
	WHERE SS.SHIPMENT_GID = SH.SHIPMENT_GID
		...
		AND SSD.S_SHIP_UNIT_GID = SSU.S_SHIP_UNIT_GID
		AND SSU.S_SHIP_UNIT_GID = SSUL.S_SHIP_UNIT_GID
		AND SSUL.ORDER_RELEASE_GID = ORLS.ORDER_RELEASE_GID) AS TIMESTAMP), 'GMT') AT TIME ZONE 'CET'),'MM')    
		and sh_temp.rate_geo_gid = T_DECLINE.RR_ID
	...
	ELSE 'NON PENALTY' END )
	else 'N/A'
	end AS 																											DAILY_CAP_FLAG,
	...
	WHERE L_ATTR.LANE_ATTRIBUTE_DEF_GID = 'ULE.ULE_NOTIFICATION_TIME_H'
		AND L_ATTR.X_LANE_GID = R_GEO.X_LANE_GID
		AND R_GEO.RATE_GEO_GID = T_DECLINE.RR_ID
		and L_ATTR.x_lane_gid not like 'ULE/SE.2Y%'
		and L_ATTR.x_lane_gid not like 'ULE/PR.XD%'),'-')) <> '-' then
	...
	WHERE SS.SHIPMENT_GID = SH.SHIPMENT_GID
		...
		AND SSD.S_SHIP_UNIT_GID = SSU.S_SHIP_UNIT_GID
		AND SSU.S_SHIP_UNIT_GID = SSUL.S_SHIP_UNIT_GID
		AND SSUL.ORDER_RELEASE_GID = ORLS.ORDER_RELEASE_GID)-T_DECLINE.TENDER_DATE)-
	...
	WHERE L_ATTR.LANE_ATTRIBUTE_DEF_GID = 'ULE.ULE_NOTIFICATION_TIME_H'
		...
		AND R_GEO.RATE_GEO_GID = T_DECLINE.RR_ID
		and L_ATTR.x_lane_gid not like 'ULE/SE.2Y%'
		and L_ATTR.x_lane_gid not like 'ULE/PR.XD%'),0)/24
		
		) ) > 0 then 'PENALTY' ELSE 'NON PENALTY' END
		else 'N/A'
		end 																																								Notification_time_PENALTY,
	...
	FROM SERVICE_TIME SERV_TIME,
		RATE_GEO R_GEO
	WHERE SERV_TIME.X_LANE_GID = R_GEO.X_LANE_GID
		AND R_GEO.RATE_GEO_GID = T_DECLINE.RR_ID),'-') <>'-' then
		case when 
		((((SELECT MIN(ORLS.early_DELIVERY_DATE)
	FROM ORDER_RELEASE ORLS,
		SHIPMENT_STOP SS,
		SHIPMENT_STOP_D SSD,
		S_SHIP_UNIT SSU,
	...
	FROM SERVICE_TIME SERV_TIME,
		RATE_GEO R_GEO
	WHERE SERV_TIME.X_LANE_GID = R_GEO.X_LANE_GID
		AND R_GEO.RATE_GEO_GID = T_DECLINE.RR_ID),0)) >-5
	...
	WHERE SERV_TIME.X_LANE_GID = R_GEO.X_LANE_GID
		...
		END 																																							
			...
			CASE
				WHEN SH_COST_BASE.SHIPMENT_COSTS_TMP_1_2 IS NULL THEN TRIM(TO_CHAR(0,'999999999D99','NLS_NUMERIC_CHARACTERS = '', '''))
				ELSE TRIM(TO_CHAR(SH_COST_BASE.SHIPMENT_COSTS_TMP_1_2 - 
			...
			from rate_geo_cost SH_TEMP
			where  SH_TEMP.RATE_GEO_COST_GROUP_GID = T_DECLINE.RR_ID
			AND SH_TEMP.CHARGE_ACTION IN ('W')
	...
	WHERE
		...
		'999999999D99','NLS_NUMERIC_CHARACTERS = '', ''')))
			FROM SHIPMENT_REFNUM SH_REF
			WHERE SH_REF.SHIPMENT_REFNUM_QUAL_GID = 'ULE.ULE_ORIGINAL_PFS' AND SH_REF.SHIPMENT_GID = SH.SHIPMENT_GID
			AND ROWNUM = 1)),'-'),'999999999D99','NLS_NUMERIC_CHARACTERS = '', '''))

				
			END  	)																																					price_final_vs_rjected,
			

		TRIM(TO_CHAR((CASE WHEN 
		(NVL((
			CASE
				WHEN SH_COST_BASE.SHIPMENT_COSTS_TMP_1_2 IS NULL THEN 0
				ELSE (SH_COST_BASE.SHIPMENT_COSTS_TMP_1_2 - 
			...
			COALESCE((SELECT
		CASE
			WHEN RGC.CHARGE_AMOUNT IS NULL THEN 0
			ELSE
				(CASE
					WHEN RGC.CHARGE_CURRENCY_GID <> 'EUR' THEN RGC.CHARGE_AMOUNT *
						(SELECT	COALESCE((SELECT EX_RATES_2_1.EXCHANGE_RATE
	...
	(SELECT
		CASE
			WHEN RGCUB.CHARGE_AMOUNT IS NULL THEN 0
			ELSE
				(CASE
					WHEN RGCUB.CHARGE_AMOUNT_GID <> 'EUR' THEN RGCUB.CHARGE_AMOUNT *
						(SELECT	COALESCE((SELECT EX_RATES_2_1.EXCHANGE_RATE
	...
	WHERE
		...
		AND RGCUB.RATE_UNIT_BREAK_GID = RUB.RATE_UNIT_BREAK_GID
		AND RUB.RATE_UNIT_BREAK_MAX = 
			(SELECT FLOOR(((case when REGEXP_LIKE(SH_REF.shipment_REFNUM_VALUE, '[^0-9 \.]') then 0
		else 
		
		(TO_NUMBER(SH_REF.shipment_REFNUM_VALUE))  end)))
			FROM SHIPMENT_REFNUM SH_REF
			WHERE SH_REF.SHIPMENT_REFNUM_QUAL_GID = 'ULE.ULE_ORIGINAL_PFS' AND SH_REF.SHIPMENT_GID = SH.SHIPMENT_GID
			AND ROWNUM = 1)),0))
	...
	WHERE
		...
		(TO_NUMBER(SH_REF.shipment_REFNUM_VALUE))  end)))
			...
			END 
		END 	),'999999999D99','NLS_NUMERIC_CHARACTERS = '', '''))																																			SUM_TO_BE_CHARGED,
	...
	WHERE
		...
		'999999999D99','NLS_NUMERIC_CHARACTERS = '', ''')))
			FROM SHIPMENT_REFNUM SH_REF
			WHERE SH_REF.SHIPMENT_REFNUM_QUAL_GID = 'ULE.ULE_ORIGINAL_PFS' AND SH_REF.SHIPMENT_GID = SH.SHIPMENT_GID
			AND ROWNUM = 1)),0)),'999999999D99','NLS_NUMERIC_CHARACTERS = '', '''))
			...
			-- and SH_TEMP.COST_TYPE = 'B'
			-- ))
			-- end ,'999999999999D99','NLS_NUMERIC_CHARACTERS = '', ''')) 		Sum_to_be_charged,
			
			
			(select listagG(TEMP.ORDER_RELEASE_REFNUM_VALUE,'/') within group (order by TEMP.ORDER_RELEASE_REFNUM_VALUE )
			from 
			
			(SELECT DISTINCT orl_ref.ORDER_RELEASE_REFNUM_VALUE,
							OM1.SHIPMENT_GID
			FROM ordeR_release_refnum orl_ref,
			ORDER_MOVEMENT OM1
			where 	orl_ref.ORDER_RELEASE_REFNUM_QUAL_GID = 'ULE.ULE_GLN'
			AND OM1.order_release_gid = orl_ref.order_release_gid
			) TEMP
			WHERE 
			 TEMP.shipment_gid = sh.shipment_gid
			)																																							GLN,
			(select listagG(TEMP.ORDER_RELEASE_REFNUM_VALUE,'/') within group (order by TEMP.ORDER_RELEASE_REFNUM_VALUE )
			from 
			...
			FROM ordeR_release_refnum orl_ref,
			ORDER_MOVEMENT OM1
			where 	orl_ref.ORDER_RELEASE_REFNUM_QUAL_GID = 'ULE.ULE_COST CENTER'
			AND OM1.order_release_gid = orl_ref.order_release_gid
			) TEMP
			WHERE 
			 TEMP.shipment_gid = sh.shipment_gid
			...
			FROM ordeR_release_refnum orl_ref,
			ORDER_MOVEMENT OM1
			where 	orl_ref.ORDER_RELEASE_REFNUM_QUAL_GID IN ('ULE.ULE_PAYER','ULE.ULE_LEG1_PAYER','ULE.ULE_LEG2_PAYER','ULE.ULE_HANDLING_PAYER')
			AND OM1.order_release_gid = orl_ref.order_release_gid
			) TEMP
			WHERE 
			 TEMP.shipment_gid = sh.shipment_gid
...
(SELECT SH_T_COLL_BOV.SHIPMENT_GID											SH_ID, 
	...
	WHERE
		...
		CASE
			...
			when SH_COST_TMP_1_2.ACCESSORIAL_CODE_GID = 'ULE/PR.ULE_FUEL_REIMBURSEMENT' THEN 1
...
WHERE
	...
	AND SH.SHIPMENT_GID = 'ULE/PR.101135621'
	
	
	-- AND TO_CHAR(From_tz(cast(T_DECLINE.REJECTION_DATE AS TIMESTAMP), 'GMT') AT TIME ZONE 'CET','YYYY') = '2015'
	-- AND TO_CHAR(From_tz(cast(T_DECLINE.REJECTION_DATE AS TIMESTAMP), 'GMT') AT TIME ZONE 'CET','MM') = NVL('07',TO_CHAR(From_tz(cast(T_DECLINE.REJECTION_DATE AS TIMESTAMP), 'GMT') AT TIME ZONE 'CET','MM'))
