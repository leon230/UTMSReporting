SELECT DISTINCT (SELECT TO_CHAR(SH_TEMP.end_time,'DD-MM-YYYY')
...
/* 		(SELECT LISTAGG(II.INVOICE_NUMBER,'/') WITHIN GROUP (ORDER BY ORLS.ORDER_RELEASE_GID)
		...
		WHERE ORLSL_TEMP.ORDER_RELEASE_LINE_GID = ILCR.	COST_REFERENCE_GID
			...
			/* 	TRIM(NVL((SELECT TO_CHAR((SUM(CASE
				...
				AND	(SH_COST_TEMP.COST_TYPE ='B' or (COST_TYPE = 'A' AND ACCESSORIAL_CODE_GID IN ('UGO.UGO_FREIGHT_COST_BASIC_FTL','UGO.UGO_FTL_FREIGHT_COST_CORRECTION',
				'UGO.UGO_FCL_FREIGHT_COST_CORRECTION'
				'UGO.UGO_FUEL_SURCHARGE_DESTINATION','UGO.UGO_FUEL_SURCHARGE_CORRECTION','UGO.FUEL_SURCHARGE_INTERMODAL_BULK_OCEAN_25','UGO.FUEL_SURCHARGE_BULK_OCEAN_24',
				'UGO.UGO_FUEL_SURCHARGE_ORIGIN','UGO.LTL_FREIGHTCOST_CORRECTION')))
				...
				AND EXCHRATES.TO_CURRENCY_GID = 'EUR')
				WHERE SH_COST_TEMP.SHIPMENT_GID=SH.SHIPMENT_GID
				AND	(COST_TYPE = 'A' AND ACCESSORIAL_CODE_GID IN ('UGO.UGO_CUSTOM_CLEARANCE_IMPORT','UGO.UGO_DEMURRAGE_LOADING_COST','UGO.UGO_DEMURRAGE_UNLOADING_COST',
				'UGO.UGO_DEMURRAGE_AT_TERMINAL','UGO.UGO_STORAGE_COSTS','UGO.UGO_CUSTOM_CLEARANCE_EXPORT','UGO.UGO_DOCUMENTATION_COST','UGO.UGO_TERMINAL_HANDLING_COST',
				'UGO.UGO_PICK_UP_FEE','UGO.UGO_DG_SURCHARGE','UGO.UGO_ADDITIONAL_CLEANING_REQUIRED','UGO.UGO_DEMURRAGE_AT_CUSTOMS','UGO.UGO_CANCELLATION_COST','UGO.UGO_CONTAINER_COOLING',
				'UGO.UGO_DETENTION','UGO.UGO_WEEKEND_COST','UGO.TERMINAL_CHANGE','UGO.UGO_TANK_CHASSIS_HIRE','UGO.SECOND_DRIVER','UGO.UGO_HEATING_COSTS','UGO.UGO_CREDIT_NOTE'))
