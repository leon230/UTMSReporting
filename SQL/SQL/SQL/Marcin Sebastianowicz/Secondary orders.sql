...
,sh.source_location_gid 														SH_source_loc_id
,s_loc.location_name															SH_source_loc_name
,s_loc_ref.LOCATION_REFNUM_VALUE												SH_source_loc_plant
,s_loc.CITY																		SH_source_loc_city
,s_loc.country_code3_gid														SH_source_loc_country
,s_loc.postal_code																SH_source_loc_postal_code
,sh.dest_location_gid 															SH_dest_loc_id
,d_loc.location_name															SH_dest_loc_name
,d_loc_ref.LOCATION_REFNUM_VALUE												SH_dest_loc_plant
,s_loc.CITY																		SH_dest_loc_city
,d_loc.country_code3_gid														SH_dest_loc_country
,d_loc.postal_code																SH_dest_loc_postal_code
,sh.servprov_gid																tsp_id
,tsp_loc.location_name															tsp_name
,orl.source_LOCATION_GID														or_source_id
,orl.dest_location_gid															or_dest_loc
...
(case
      ...
      )                                                                                                                                                                        SH_CATEGORY_lane_level
,(UPPER((SELECT listagg(NVL(loc_ref.location_refnum_value,'n/a'),'/') within group (order by sh.source_location_gid)
                  FROM location_refnum loc_ref
                  WHERE loc_ref.location_gid = sh.DEST_LOCATION_GID
                  AND loc_ref.location_refnum_qual_gid = 'ULE.ULE_CATEGORY'))
                  )	  																																								SH_dest_loc_category
,(UPPER((SELECT listagg(NVL(loc_ref.location_refnum_value,'n/a'),'/') within group (order by sh.source_location_gid)
                  FROM location_refnum loc_ref
                  WHERE loc_ref.location_gid = sh.source_LOCATION_GID
                  AND loc_ref.location_refnum_qual_gid = 'ULE.ULE_CATEGORY'))
                  )	  																																								SH_source_loc_category				  
...
SUM(case
		...
		WHERE alloc_d.order_release_gid = orl.order_release_gid
		...
		AND alloc_d.COST_DESCRIPTION in ('B')
...
SUM(case
		...
		AND ab.shipment_gid = sh.shipment_gid
		-- AND alloc_d.IS_WEIGHTED = 'N'
		AND alloc_d.COST_DESCRIPTION in ('A')
...
,NVL((SELECT 
		...
		WHERE SH_TEMP.SHIPMENT_GID = SH.SHIPMENT_GID),0)																											sh_TOTAL_DISTANCE
...
,ROUND(orl.total_weight_base*0.45359237,2)																																PALLET_GROSS_WEIGHT_KG

,ROUND(
case when sh.total_num_reference_units > 0 then sh.total_num_reference_units
else sh.total_ship_unit_count end,2)																								                        SH_PFS
...
,order_movement om
,order_release orl
,order_release or_est_pfs
LEFT OUTER join order_release_refnum or_pfs_est on (or_est_pfs.order_release_gid = or_pfs_est.order_release_gid AND or_pfs_est.order_release_refnum_qual_gid = 'ULE.ULE_ESTIMATED_PFS')
,order_release or_est_u
LEFT OUTER join order_release_refnum or_pfs_est_u on (or_est_u.order_release_gid = or_pfs_est_u.order_release_gid AND or_pfs_est_u.order_release_refnum_qual_gid = 'ULE.ULE_UPDATED_ESTIMATED_PFS')
,order_release or_pfs_org
LEFT OUTER join order_release_refnum orref_pfs_org on (or_pfs_org.order_release_gid = orref_pfs_org.order_release_gid AND orref_pfs_org.order_release_refnum_qual_gid = 'ULE.ULE_ORIGINAL_PFS')



WHERE 
sh.start_time >= to_date('2016-05-15','YYYY-MM-DD') 
...
AND or_est_pfs.order_release_gid = orl.order_release_gid
AND or_est_u.order_release_gid = orl.order_release_gid
AND or_pfs_org.order_release_gid = orl.order_release_gid
